...
function makeProwlarrWebhookEvent(ServiceConnection $serviceConnection, string $eventType, array $extra = []): WebhookEvent
{
...
test('Health event writes an ActivityLog row and dispatches a health re-ping', function (): void {
    ...
    $webhookEvent = makeProwlarrWebhookEvent($this->connection, 'Health', [
        'level' => 'warning',
...
test('HealthRestored event writes an ActivityLog row and dispatches a health re-ping', function (): void {
    ...
    $webhookEvent = makeProwlarrWebhookEvent($this->connection, 'HealthRestored', [
        'level' => 'ok',
        'type' => 'IndexerLongTermStatusCheck',
...
test('ApplicationUpdate event writes an ActivityLog row and dispatches a version re-fetch', function (): void {
    ...
    $webhookEvent = makeProwlarrWebhookEvent($this->connection, 'ApplicationUpdate', [
...
test('unknown event types are ignored without writing ActivityLog', function (): void {
    $webhookEvent = makeProwlarrWebhookEvent($this->connection, 'NotARealEvent');
