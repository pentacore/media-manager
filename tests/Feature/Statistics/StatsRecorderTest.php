...
it('increments hour and day buckets additively', function (): void {
    ...
    $statRollup = StatRollup::query()->where('period', 'hour')->sole();
    $day = StatRollup::query()->where('period', 'day')->sole();

    expect($statRollup->count)->toBe(3)
        ->and($statRollup->bucket->toIso8601String())->toBe('2026-07-06T14:00:00+00:00')
...
it('maintains gauge count sum min max via sample', function (): void {
    ...
    expect($statRollup->count)->toBe(3)
        ->and($statRollup->sum)->toBe(18.0)
        ->and($statRollup->min)->toBe(2.0)
        ->and($statRollup->max)->toBe(11.0);
});
...
it('put overwrites an existing bucket instead of adding', function (): void {
    ...
    $statRollup = StatRollup::query()->sole();

    expect($statRollup->count)->toBe(7)->and($statRollup->sum)->toBe(123.0);
});
