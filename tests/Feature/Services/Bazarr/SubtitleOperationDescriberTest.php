...
test('download_best names the media and the requested subtitle', function (): void {
    $actionDescription = resolve(SubtitleOperationDescriber::class)->describe(
        'download_best',
        ['title' => 'Severance S01E01'],
        ['language' => 'sv', 'forced' => false, 'hearing_impaired' => true],
    );

    expect($actionDescription->title)->toBe('Download the best subtitle for Severance S01E01')
        ->and($actionDescription->description)->toBe('Bazarr will search its providers and download the best-scoring subtitle.')
        ->and($actionDescription->details)->toBe([
...
test('track operations name the selected subtitle track', function (): void {
    ...
    expect($actionDescription->title)->toBe('Delete a subtitle for Dune')
        ->and($actionDescription->details)->toContain(['label' => 'Subtitle', 'value' => 'English (forced)']);
});
...
test('an upload is described with its language', function (): void {
    ...
    expect($actionDescription->title)->toBe('Upload a subtitle for Dune')
        ->and($actionDescription->description)->toBe('Bazarr will add the uploaded subtitle file to this media.');
});
