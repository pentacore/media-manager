...
const myLink = computed<UserLink | null>(() => {
    ...
    return props.links.find((link) => link.user?.id === currentUserId.value) ?? null
})
    ...
    <div class="space-y-6 p-6">
        <div>
            ...
            <p class="text-muted-foreground">
                Connect your application account to an Emby user to track playback activity.
            </p>
        </div>
        ...
        <Card>
            <CardHeader>
                ...
                <CardDescription>
                    {{ myLink ? 'Your account is linked.' : 'Link your Emby account to your profile.' }}
                </CardDescription>
            </CardHeader>
            <CardContent>
                <div v-if="myLink" class="flex items-center justify-between gap-4">
                    <div>
                        <p class="font-medium">{{ myLink.emby_username }}</p>
                        <p class="text-xs text-muted-foreground">Linked {{ formatDate(myLink.created_at) }}</p>
                    </div>
                    <Button variant="destructive" size="sm" @click="revoke(myLink)">
                ...
                >
                    <div class="space-y-2">
                        ...
                        <Input id="emby_username" name="emby_username" required placeholder="Your Emby username" />
                        <InputError :message="errors.emby_username" />
                    </div>
                    <div class="space-y-2">
                        <Label for="emby_password">Emby Password</Label>
                        <PasswordInput id="emby_password" name="password" required placeholder="Your Emby password" />
        ...
        <Card v-if="isAdmin">
            <CardHeader>
                ...
                <CardDescription>
                    {{ (props.links?.length ?? 0) }} linked {{ (props.links?.length ?? 0) === 1 ? 'account' : 'accounts' }}
                </CardDescription>
            </CardHeader>
            <CardContent>
                <Table>
                    ...
                    <TableBody>
                        <TableRow v-for="link in (props.links ?? [])" :key="link.id">
                            <TableCell class="font-medium">{{ link.emby_username }}</TableCell>
                            <TableCell>
                                <span v-if="link.user">
                                    {{ link.user.name }}
                                    <span class="block text-xs text-muted-foreground">{{ link.user.email }}</span>
                                </span>
                            ...
                            <TableCell class="text-right">
                                <Button variant="ghost" size="sm" @click="revoke(link)">
                                    <Trash2 class="mr-2 size-4" />
                                    Revoke
                                </Button>
                            </TableCell>
