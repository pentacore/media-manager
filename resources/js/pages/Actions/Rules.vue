...
defineOptions({
    layout: {
        breadcrumbs: [
            ...
            { title: 'Action Rules', href: ActionTypeConfigController.index.url() },
        ],
    },
})
    ...
    <div class="space-y-6 p-6">
        <div>
            ...
            <p class="text-muted-foreground">
                Configure which automated actions require approval and which are enabled.
            </p>
        </div>

        <div v-if="localRules.length === 0" class="rounded-md border p-8 text-center text-muted-foreground">
        ...
        <div v-else class="grid grid-cols-1 gap-4 md:grid-cols-2">
            <Card v-for="rule in localRules" :key="rule.id">
                <CardHeader>
                    ...
                    <p class="text-xs font-mono text-muted-foreground">{{ rule.type }}</p>
                </CardHeader>
                <CardContent class="space-y-4">
                    ...
                    <div class="flex items-center justify-between">
                        <label :for="`approval-${rule.id}`" class="text-sm font-medium">
                            Requires approval
                        </label>
                        <Checkbox
                            :id="`approval-${rule.id}`"
                            :model-value="rule.requires_approval"
                            @update:model-value="(v) => onApprovalChange(rule, v)"
                        />
                    </div>

                    <div class="flex items-center justify-between">
                        <label :for="`enabled-${rule.id}`" class="text-sm font-medium">
                            Enabled
                        </label>
                        <Checkbox
                            :id="`enabled-${rule.id}`"
                            :model-value="rule.is_enabled"
                            @update:model-value="(v) => onEnabledChange(rule, v)"
                        />
                    </div>
                </CardContent>
            </Card>
