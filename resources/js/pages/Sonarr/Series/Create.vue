    ...
    <div class="space-y-6 p-6">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-muted-foreground">Search for a series on TVDB and add it to Sonarr.</p>
            </div>
        ...
        <div v-if="!searchTerm && searchResults.length === 0" class="rounded-md border bg-muted/30 p-8 text-center text-muted-foreground">
            Search for a series to add.
        </div>

        <div v-else-if="searchResults.length === 0" class="rounded-md border bg-muted/30 p-8 text-center text-muted-foreground">
            ...
            >
                <div class="flex gap-3 p-4">
                    ...
                    <div class="min-w-0 flex-1">
                        <h3 class="truncate font-semibold" :title="result.title ?? ''">
                            {{ result.title ?? 'Unknown' }}
                        </h3>
                        <p v-if="result.year" class="text-sm text-muted-foreground">{{ result.year }}</p>
                        <p v-if="result.overview" class="mt-1 line-clamp-3 text-xs text-muted-foreground">
                            {{ result.overview }}
                        </p>
                    </div>
                </div>

                <CardContent v-if="selectedTvdbId !== result.tvdb_id" class="pt-0">
                    <Button
                        class="w-full"
                        size="sm"
                ...
                <CardContent v-else class="space-y-3 pt-0">
                    <div class="space-y-1">
                        <Label :for="`quality-${result.tvdb_id}`">Quality Profile</Label>
                    ...
                    <div class="space-y-1">
                        <Label :for="`root-${result.tvdb_id}`">Root Folder</Label>
