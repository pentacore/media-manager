...
final class BazarrSubtitleFingerprint
{
     ...
     */
    public function make(array $subtitle): string
    {
        return hash_hmac(
            'sha256',
            json_encode(
                $this->canonicalize(array_intersect_key($subtitle, array_flip(self::FIELDS))),
     ...
     * @throws JsonException
     */
    public function verify(array $subtitle, string $fingerprint): bool
    {
        return preg_match('/^[a-f0-9]{64}$/D', $fingerprint) === 1
            && hash_equals($this->make($subtitle), $fingerprint);
    }
