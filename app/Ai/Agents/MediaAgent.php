...
3. **Take individual actions.** Two flavors:
   ...
   **Stuck downloads (manual intervention required):**
   ...
   - To discard: RemoveStuckDownloadChatTool (queues an action; deletes the downloaded data). Pass blocklist=true when the release itself is bad (corrupt/fake/wrong content) so it is never grabbed again; leave it off when the release merely isn't an upgrade. Pass search_replacement=true when a replacement should be grabbed (e.g. blocklisted a bad release but the user still wants the content); leave it false when the content shouldn't be retried.
