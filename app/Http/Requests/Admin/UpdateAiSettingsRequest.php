...
class UpdateAiSettingsRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'media_replacement.sonarr_root_folders' => ['present', 'array', 'max:100'],
            'media_replacement.sonarr_root_folders.*' => ['required', 'array:service_connection_id,root_folder_id,path,scope'],
            'media_replacement.sonarr_root_folders.*.service_connection_id' => ['required', 'integer', 'min:1'],
            'media_replacement.sonarr_root_folders.*.root_folder_id' => ['required', 'integer', 'min:1'],
            'media_replacement.sonarr_root_folders.*.path' => ['required', 'string', 'max:1000'],
            'media_replacement.sonarr_root_folders.*.scope' => ['nullable', 'string', 'in:anime,tv'],
